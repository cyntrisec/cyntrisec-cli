...
from cyntrisec.core.schema import Finding, Asset
...
CIS_CONTROLS = [
    # IAM
    Control("1.4", Framework.CIS_AWS, "Ensure no root account access key exists", 
            "The root account should not have access keys configured", "critical"),
    Control("1.5", Framework.CIS_AWS, "Ensure MFA is enabled for root account",
            "The root account should have MFA enabled", "critical"),
    Control("1.10", Framework.CIS_AWS, "Ensure MFA is enabled for all IAM users with console password",
            "All IAM users with console access should have MFA enabled", "high"),
    Control("1.12", Framework.CIS_AWS, "Ensure credentials unused for 90 days are disabled",
            "IAM credentials not used in 90 days should be disabled", "medium"),
    Control("1.16", Framework.CIS_AWS, "Ensure IAM policies not attached directly to users",
            "IAM policies should be attached to groups/roles, not users", "medium"),
    Control("1.17", Framework.CIS_AWS, "Ensure wildcard (*) not used in IAM policies",
            "IAM policies should not use wildcards for resources", "high"),
    
    # S3
    Control("2.1.1", Framework.CIS_AWS, "Ensure S3 bucket Block Public Access is enabled",
            "All S3 buckets should have Block Public Access enabled", "high"),
    Control("2.1.2", Framework.CIS_AWS, "Ensure S3 bucket Block Public Access at account level",
            "Account-level S3 Block Public Access should be enabled", "high"),
    Control("2.1.5", Framework.CIS_AWS, "Ensure S3 bucket access logging is enabled",
            "S3 buckets should have access logging enabled", "medium"),
    
    # EC2/VPC
    Control("5.1", Framework.CIS_AWS, "Ensure no open Security Groups to 0.0.0.0/0",
            "Security groups should not allow 0.0.0.0/0 ingress", "high"),
    Control("5.2", Framework.CIS_AWS, "Ensure default security group restricts all traffic",
            "VPC default security groups should not allow any traffic", "medium"),
    Control("5.3", Framework.CIS_AWS, "Ensure VPC flow logging is enabled",
            "All VPCs should have flow logging enabled", "medium"),
    Control("5.4", Framework.CIS_AWS, "Ensure EC2 instances use IMDSv2",
            "EC2 instances should use Instance Metadata Service v2", "medium"),
]

# SOC 2 Type II Controls
SOC2_CONTROLS = [
    Control("CC6.1", Framework.SOC2, "Logical and Physical Access Controls",
            "Access to system components is controlled by access policies", "high"),
    Control("CC6.2", Framework.SOC2, "Prior to Access",
            "Users are authenticated before access is granted", "high"),
    Control("CC6.3", Framework.SOC2, "Role-Based Access",
            "Access is based on job function and least privilege", "high"),
    Control("CC6.6", Framework.SOC2, "Encryption of Data",
            "Data at rest and in transit is encrypted", "high"),
    Control("CC6.7", Framework.SOC2, "Data Disposal",
            "Data is disposed of securely when no longer needed", "medium"),
    Control("CC7.1", Framework.SOC2, "Security Monitoring",
            "Security events are detected and responded to", "high"),
    Control("CC7.2", Framework.SOC2, "Incident Response",
            "Security incidents are managed and resolved", "high"),
]
