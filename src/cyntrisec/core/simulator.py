...
class OfflineSimulator:
    ...
    def can_access(
        ...
        *,
        ...
        """
        # Find assets
        principal = self._assets.get(principal_arn) or self._assets_by_name.get(principal_arn.split("/")[-1])
        ...
        if principal and target:
            ...
            for rel in self._relationships:
                if (rel.source_asset_id == principal.id and 
                    rel.target_asset_id == target.id and
                    rel.relationship_type in ("MAY_ACCESS", "CAN_ASSUME", "ALLOWS")):
                    can_access = True
                    proof = {
